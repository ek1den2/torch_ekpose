...
def draw_humans(npimg, humans, imgcopy=False):
    ...
    for human in humans:
        ...
        for i in range(CocoPart.Background.value):
            ...
            centers[i] = center
            cv2.circle(npimg, center, 1, CocoColors[i], thickness=max(1, int(10*scale)), lineType=8)

        # 線を描画（最後2つは描画しない）
        for pair_order, pair in enumerate(CocoPairsRender):
            if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                continue
            cv2.line(npimg, centers[pair[0]], centers[pair[1]], CocoColors[pair_order], max(1, int(2*scale)))
